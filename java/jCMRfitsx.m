function [p, datafit, fits] = jCMRfitsx(nsample,y, model, E, shrink, proc)
...
if iscell(y)
    if isstruct(y{1})
        ...
        for ivar = 1:nvar
            for igroup = 1:ngroup
                ncond = ncond + size(y{igroup, ivar},2);
            end
        end        
    end
else
    ...
    ncond = numel(unique(y(:,2))) * (size(y,2) - 3);
end
...
if iscell(y)
    if isstruct(y{1})
        ...
        sol = CMRxFits(nsample,problem.getProblem(),shrink,proc);
    else
        for group=1:ngroup
            for var=1:nvar
        ...
        sol = problem.solve(nsample,proc);
    end
else
    problem.setGM(y);
    sol = problem.solve(nsample,proc);
