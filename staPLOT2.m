...
if iscell(data)
    if isstruct(data{1})
        ...
        ys = staSTATS(data, 0); % otherwise assume within-subjects data and get stats
    end
else
    celldata = gen2cell (data); % convert from old "general format"
    ys = staSTATS (celldata, 0);
end
...
for igroup = 1:numel(groups)
    ...
    b = y(groups{igroup});
    if igroup==1
        h=plot (a, b, 'o', 'markerfacecolor', hex2rgb('#D55E00'), 'markersize', msize); hold on
    elseif igroup==2
        h=plot (a, b, 'o', 'markerfacecolor', hex2rgb('#E69F00'), 'markersize', msize);
    elseif igroup==3
        h=plot (a, b, 'o', 'markerfacecolor', hex2rgb('#56B4E9'), 'markersize', msize);
    elseif igroup==4
        h=plot (a, b, 'o', 'markerfacecolor', hex2rgb('#009E73'), 'markersize', msize);
    elseif igroup==5
        h=plot (a, b, 'o', 'markerfacecolor', colour, 'markersize', msize);
    else 
        h=plot (a, b, 'o', 'markerfacecolor', 'w', 'markersize', msize);
    end
    if flag
        DeleteLegendEntry (h);
    end 
